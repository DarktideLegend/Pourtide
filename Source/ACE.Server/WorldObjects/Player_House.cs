    ...
    partial class Player
    {
        ...
        public void HandleActionBuyHouse(ObjectGuid slumlord_id, List<uint> item_ids)
        {
            ...
            if (!TryConsumePurchaseItems(consumeItems))
            {
                ...
                log.ErrorFormat("[HOUSE] {0}.HandleActionBuyHouse({1:X16}, {2}) - TryConsumePurchaseItems failed with {3}", Name, slumlord_id, item_id_list, consumeItemsList);
        ...
        public void HandleActionRentHouse(ObjectGuid slumlord_id, List<uint> item_ids)
        {
            ...
            if (owner != null)
            {
                ...
                log.ErrorFormat("[HOUSE] {0}.HandleActionRentHouse({1:X16}): couldn't find house owner {2}", Name, slumlord_id, slumlord.HouseOwner);
        ...
        public List<WorldObject> GetInventoryItems(List<uint> item_ids)
        {
            ...
            foreach (var item_id in item_ids)
            {
                ...
                if (item != null)
                    ...
                    log.ErrorFormat("{0}.GetInventoryItems() - couldn't find {1:X16}", Name, item_id);
            }
